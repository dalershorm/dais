  ...
  <div class="px-6">
    <div class="w-full py-8">
      <div class="flex items-center justify-between w-full">
        <div class="">
          <h2 class="md:text-4xl text-2xl font-bold text-gray-700 mb-2.5">Тип доставки</h2>
          <span class="text-gray-800">Список типов доставки.</span>
        </div>
        <div class="flex">
          <nuxt-link to="/shippings/create" class="d-btn d-btn-primary">
        ...
        <vue-good-table
          ...
          <template slot="table-row" slot-scope="props">
            <span class="flex justify-center space-x-5" v-if="props.column.field == 'action'">
                <nuxt-link :to="`/shippings/edit/${props.row.id}`"
              ...
              <span v-else-if="props.column.field == 'created_at'">
                   <span class="text-sm">{{ moment(String(props.row.updated_at)).format('LLL') }}</span>
              </span>
          </template>
...
export default {
  ...
  methods: {
    init() {
      this.loading = true
      this.$axios.get('/dashboard/shippings')
      .then(res=> {
        this.rows = res.data.shippings
        this.loading = false
      })
    },
    removeItem(item) {
      this.$swal({
        ...
        this.$axios.delete('/dashboard/shippings/' + item.id).then(res => {
          this.init()
        })
      })
    }
