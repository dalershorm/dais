    ...
    <div class="px-6 w-full relative z-0 overflow-y-auto focus:outline-none" v-if="report">
        <div class="py-8">
          <div class="flex items-center justify-between w-full">
            <div class="">
              <h2 class="md:text-3xl text-xl font-bold text-gray-700 mb-2.5">Ежемесячный расход</h2>
              <span class="text-gray-800">Режим просмотр ежемесячного расхода.</span>
            </div>
            <div class="flex">
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <div class="px-4 py-5 border-b border-gray-200 sm:px-6 flex justify-between items-center">
                <div>
                    <h3 class="flex items-center text-lg leading-6 font-medium text-gray-900">
                        <span class="mr-3">Карточка расхода: </span>
            ...
            <div class="px-4 py-5 sm:p-0 mb-10">
                <dl>
                    ...
                    <div class="mt-8 sm:mt-0 sm:grid sm:grid-cols-3 sm:gap-4 sm:border-t sm:border-gray-200 sm:px-6 sm:py-5">
                        <dt class="text-sm leading-5 font-medium text-gray-500">
                            Дата <span class="lowercase">расхода</span>:
                        </dt>
                        <dd class="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2">
...
export default {
    ...
    methods: {
        init() {
            this.$axios.get('/dashboard/report-monthly/' + this.$route.params.id)
            .then(res=> {
                this.report = res.data.report
                this.orders = this.report.monthly_report_orders
                this.loading = false
            })
        },
        closeReport(id) {
            this.$axios.post('/dashboard/report-monthly-status/' + id, {status_id: 19})
            .then(res=> {
                this.$swal({
                    icon: 'success',
                    title: 'Успешно',
                    text: 'Расход успешно закрыт',
                    timer: 2000,
                ...
                this.$swal({
                    icon: 'erros',
                    title: 'Ошибка',
                    text: "Возникла ошибка при закрытии расхода",
                    timer: 2000,
                    timerProgressBar: true,
                })
            })
