    ...
    <div>
        ...
        <div class="sm:hidden mt-10">
            <div class="px-4 sm:px-6">
                <h2 class="text-gray-500 text-xs font-medium uppercase tracking-wide">Последние заказы</h2>
            </div>
            <ul class="mt-3 border-t border-gray-200 divide-y divide-gray-100" v-if="orders.length > 0">
                <li v-for="(item, index) in orders" :key="index">
                    <nuxt-link :to="`/profile/orders/${item.track_code}`" class="flex items-center justify-between px-4 py-4 hover:bg-gray-50 sm:px-6">
                        <div class="flex items-center truncate space-x-3">
                            <div class="w-2.5 h-2.5 flex-shrink-0 rounded-full" :class="item.status_id == 1 ? 'bg-blue-600' : (item.status_id == 2 ? 'bg-purple-600' : (item.status.id == 3 ? 'bg-yellow-600' : 'bg-green-600'))"></div>
                            <p class="font-medium truncate text-sm leading-6">{{ item.track_code }} <span class="truncate font-normal text-gray-500">| Вес: {{ item.weight }} (кг) | Место: {{ item.boxes }}</span></p>
                        </div>
                ...
                <table class="min-w-full">
                    <thead>
                        ...
                        <th class="px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                            <span class="lg:pl-2">Трек Код</span>
                        </th>
                        <th class="px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                            Место
                        </th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-100" v-if="orders.length > 0">
                        <tr v-for="(item, index) in orders" :key="index">
                            <td class="px-6 py-3 max-w-0 w-full whitespace-no-wrap text-sm leading-5 font-medium text-gray-900">
                                <div class="flex items-center space-x-3 lg:pl-2">
                                    <div class="flex-shrink-0 w-2.5 h-2.5 rounded-full" :class="item.status_id == 1 ? 'bg-blue-600' : (item.status_id == 2 ? 'bg-purple-600' : (item.status.id == 3 ? 'bg-yellow-600' : 'bg-green-600'))"></div>
                                    <nuxt-link :to="`/profile/orders/${item.track_code}`" class="truncate hover:text-gray-600">
                                        <span>{{ item.track_code }}</span>
                                    </nuxt-link>
                                </div>
                            </td>
                            ...
                            <td class="py-3 text-sm leading-5 text-gray-500 font-medium text-center">
                                <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full whitespace-nowrap" :class="item.status_id == 1 ? 'bg-blue-100 text-blue-800' : (item.status_id == 2 ? 'bg-purple-100 text-purple-800' : (item.status.id == 3 ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'))">
                                    {{ item.status.name }}
                                </span>
                            </td>
